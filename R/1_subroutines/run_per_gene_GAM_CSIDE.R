...
per_gene_GAM_CSIDE=function(abs_expr,sample_info,size.factor,gene.name){

# per_gene_GAM_CSIDE=function(abs_expr,sample_info,gene.name){

stt <- Sys.time()
sm <- mgcv::smoothCon(mgcv::s(x, y, fx = T, bs ='tp'), data = as.data.frame(sample_info))[[1]]
